        ...
        when (chunkName) {
            ...
            "XYZI" -> {
                ...
                voxels.add(Array(numVoxels) {
                    ...
                    Voxel(x.toInt(), y.toInt(), z.toInt(), Color((p and 0xff).toInt(), ((p shr 8) and 0xff).toInt(), ((p shr 16) and 0xff).toInt()))
                })
...
fun denseVoxels(voxFile: VoxFile) : Array<Color> {
    val denseVox = Array(voxFile.width * voxFile.height * voxFile.depth) { Color(-1, -1, -1) }
    voxFile.voxels.forEach {
        denseVox[it.x + it.y * voxFile.width + it.z * voxFile.width * voxFile.height] = it.color
    }
    return denseVox
}
